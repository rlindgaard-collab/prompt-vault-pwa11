...
export default function App(){
  const [sheetLink, setSheetLink] = useState(() => localStorage.getItem("pv42_sheet") || DEFAULT_LINK);
  const [tabs, setTabs] = useState([]); // [{gid,name,csvUrl}]
  const [activeGid, setActiveGid] = useState(() => localStorage.getItem("pv42_gid") || "");
  const [rows, setRows] = useState([]); // raw rows (arrays)
  const [category, setCategory] = useState(() => localStorage.getItem("pv42_cat") || "");
  const [q, setQ] = useState("");

  const activeTab = useMemo(()=>tabs.find(t=>String(t.gid)===String(activeGid)), [tabs, activeGid]);

  useEffect(()=>localStorage.setItem("pv42_sheet", sheetLink),[sheetLink]);
  useEffect(()=>localStorage.setItem("pv42_gid", String(activeGid||"")),[activeGid]);
  useEffect(()=>localStorage.setItem("pv42_cat", category),[category]);
  ...
  const categories = useMemo(()=>{
    const s = new Set();
    for(const r of rows){ const v=(r[0]||"").trim(); if(v) s.add(v); }
    return Array.from(s);
  },[rows]);
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      <header className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Prompt Vault v4.2</h1>
...
function extractTabs(html, baseCsv){
  ...
  if(out.length===0){
    out.push({ gid: "0", name:"Fane 1", csvUrl: baseCsv });
  }
  return out;
}
...
// Try to skip headers/banners to first real row
function findDataStart(rows){
  for(let r=0; r<Math.min(30, rows.length); r++){
    ...
    const looksHeader = ["kategori","category","type","label","navn","title","overskrift"].includes(a) ||
                        ["prompt","tekst","text"].includes(b);
    if(nonEmpty>=1){
      if(looksHeader) return r+1;
      const next1 = (rows[r+1]||[])[1] || "";
      const next2 = (rows[r+2]||[])[1] || "";
      if((rows[r][1]||"").toString().trim() || next1.toString().trim() || next2.toString().trim()) return r;
    }
  }
  return 0;
}
